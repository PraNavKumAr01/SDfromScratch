...
class DDPMSampler:
    ...
    def get_prev_timestep(self, timestep: int):
    ...
    def add_noise(self, original_sample: torch.FloatTensor, timestep: torch.IntTensor) -> torch.FloatTensor:
        alpha_cumprod = self.alpha_cumprod.to(device = original_sample.device, dtype = original_sample.dtype)
        timestep = timestep.to(original_sample.device)
